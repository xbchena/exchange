...
@Component("single")
final class RedisCacheImpl implements RedisCache {
				...
				if (bytes != null) {
					try {
						...
						if (logger.isDebugEnabled()) {
							//日志开到debug级别才printStackTrace,防止刷日志
							e.printStackTrace();
						}
	...
	public <T> boolean remove(final BaseCacheKey key) {
		return template.execute(new RedisCallback<Boolean>() {
			...
			public Boolean doInRedis(RedisConnection connection) throws DataAccessException {
				try {
					connection.del(encodeKey(key.getKey(), callerName));
					return true;
				} catch (Exception e) {
					e.printStackTrace();
					return false;
	...
	public void init() {
		try {
			...
			if (CollectionUtils.isNotEmpty(uapConfig.master) && uapConfig.master.size() == 1
					&& CollectionUtils.isEmpty(uapConfig.slave) && CollectionUtils.isEmpty(uapConfig.sentinel)) {
				...
				JedisConnectionFactory fac = new JedisConnectionFactory(pollConfig);
